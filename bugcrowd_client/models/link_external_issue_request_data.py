...
class LinkExternalIssueRequestData(BaseModel):
    """
    LinkExternalIssueRequestData
    """ # noqa: E501
    type: StrictStr
    attributes: LinkExternalIssueRequestDataAttributes
    relationships: LinkExternalIssueRequestDataRelationships
    __properties: ClassVar[List[str]] = ["type", "attributes", "relationships"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('external_issue'):
            raise ValueError("must be one of enum values ('external_issue')")
        return value
    ...
    def from_dict(cls, obj: Dict) -> Self:
        ...
        _obj = cls.model_validate({
            "type": obj.get("type"),
            "attributes": LinkExternalIssueRequestDataAttributes.from_dict(obj.get("attributes")) if obj.get("attributes") is not None else None,
            "relationships": LinkExternalIssueRequestDataRelationships.from_dict(obj.get("relationships")) if obj.get("relationships") is not None else None
        })
        return _obj
